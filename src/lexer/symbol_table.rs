...
#[fail(display = "invalid symbol table. first: {:?}, second: {:?}", first, second)]
pub struct InvalidSymbolTableError {
    pub first: Vec<String>,
    pub second: Vec<String>,
}

/// Check that a symbol table can be used to parse unambiguously.
pub fn validate_symbol_table<'s>(symbols: &[&'s str]) -> Result<(), InvalidSymbolTableError> {
    fn recurse<'s>(symbols: &[&'s str],
                   complete_list: &mut Vec<&'s str>,
                   postfix_list: &mut Vec<&'s str>,
                   postfix: &'s str) -> Result<(), InvalidSymbolTableError>
    {
            ...
            if postfix.is_prefix_of(symbol) {
                ...
                if new_postfix == "" {
                    return Err(InvalidSymbolTableError {
                        first: complete_list.iter().map(|s| String::from(*s)).collect(),
                        second: postfix_list.iter().map(|s| String::from(*s)).collect(),
                    })
                }
                recurse(symbols, postfix_list, complete_list, new_postfix)?;
